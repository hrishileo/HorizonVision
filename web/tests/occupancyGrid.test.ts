...
describe("BEV occupancy grid", () => {
  ...
  it("keeps an occupied cell after the sweep no longer covers it", () => {
    ...
    expect(cellState(first, cell.ix, cell.iy)).toBe("occupied");

    const empty = rasterizeSweep(new Float32Array(0), new Int32Array(0), 40, [], tiny);
    expect(empty.states.every((s) => s === CELL_UNKNOWN)).toBe(true);

    const merged = integrateSweep(first, empty);
    expect(cellState(merged, cell.ix, cell.iy)).toBe("occupied");
    expect(merged.occupied.some((c) => c.ix === cell.ix && c.iy === cell.iy)).toBe(true);
    ...
    expect(cellState(stillOcc, cell.ix, cell.iy)).toBe("occupied");
  });
});
